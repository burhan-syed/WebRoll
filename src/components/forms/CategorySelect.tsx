...
export default function CategorySelect({
  ...
  useEffect(() => {
    ...
    if(maxCount && count > maxCount){
      setError("categories", {type: "maxLength", message: "select no more than two"})
    }
    else if(minCount && (!count || count < minCount)){
      setError("categories", {type: "minLength", message: "select at least one"})
    }
    else if(minCount && maxCount && value.includes("Fun") && count < 2){
      setError("categories", {type: "minLength", message: `select two when "Fun" is selected`})
  ...
  return (
    <>
      {categories.map((category,i) => (
        ...
        >
          <input
            className={"checkbox mt-1.5" + (value.includes(category.category) ? " checkbox-primary " : "")}
            type={"checkbox"}
