...
export default function ExplorePage({
  ...
  useEffect(() => {
    ...
    if (sites[index].allowEmbed === false ) {
      setAdvanced(p => p+1)
      siteImgURL.url !== sites[index].url && fetchSecureImage(sites[index].url, sites[index].imgKey);
    }
  }, [index]);
  ...
  return (
    <>
      <nav className="min-w-full fixed bottom-0 md:bottom-auto md:top-0 z-50">
        <ExploreNavBar site={sites[index]} advance={advance} advanced={advanced}  />
      </nav>
      {/* <div className="relative flex flex-grow  overflow-y-auto order-1 md:order-2"> */}
      <div className="fixed bottom-1/2 left-0 z-20 text-xl bg-black text-white">{index},{sites[index].allowEmbed ? "allowed" : "nope"}</div>
        ...
        <div className="flex-grow flex flex-col">
          {sites[index].allowEmbed === true ? (
            <iframe
              ...
              src={sites[index].url}
              onLoad={() => {console.log("iframe loaded"); setAdvanced(p => p+1)}}
              onError={() => {console.log("iframe error"); setAdvanced(p => p+1)}}
            ></iframe>
